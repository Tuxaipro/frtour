...
@section('subtitle', 'Manage destination locations')
...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Destinations</h1>
            <p class="text-sm text-slate-600 mt-1.5 font-medium">Manage destination locations for your tours</p>
        </div>
    ...
    <form method="GET" action="{{ route('admin.destinations.index') }}" class="bg-white rounded-2xl shadow-lg border border-slate-200/50 p-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search destinations..." class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-all duration-200 text-slate-900 placeholder-slate-400">
            ...
            <div class="flex space-x-3">
                ...
                <a href="{{ route('admin.destinations.index') }}" class="flex-1 bg-white hover:bg-slate-50 text-slate-700 border-2 border-slate-200 px-5 py-3 rounded-xl font-semibold transition-all duration-200 text-center shadow-sm hover:shadow-md">Clear</a>
            </div>
        </div>
    </form>
    ...
    @if($destinations->count() > 0)
        <div class="bg-white rounded-2xl shadow-lg border border-slate-200/50 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white">
                <h2 class="text-lg font-bold text-slate-900">All Destinations</h2>
                <p class="text-xs text-slate-600 mt-1">Total: {{ $destinations->count() }} destination(s)</p>
            </div>
                ...
                <tbody class="bg-white divide-y divide-slate-200">
                    @foreach($destinations as $destination)
                        <tr class="hover:bg-primary/5 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-slate-500">{{ $destination->slug }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                                {{ $destination->sort_order ?? 0 }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $destination->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    <a href="{{ route('admin.destinations.show', $destination) }}" 
                                       class="text-primary hover:text-primary-dark p-2 rounded-lg hover:bg-primary/10 transition-all duration-200" title="View">
                                    ...
                                    <form action="{{ route('admin.destinations.duplicate', $destination) }}" 
                                          method="POST" 
                                          class="inline">
                                        @csrf
                                        <button type="submit" class="text-purple-600 hover:text-purple-700 p-2 rounded-lg hover:bg-purple-50 transition-all duration-200" title="Duplicate">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @else
        <div class="bg-white rounded-2xl shadow-lg border border-slate-200/50 p-12 text-center">
            <div class="max-w-md mx-auto">
                ...
                <h3 class="text-xl font-bold text-slate-900 mb-2">No destinations found</h3>
                <p class="text-sm text-slate-600 mb-8">Start by adding a destination to organize your tours.</p>
                <a href="{{ route('admin.destinations.create') }}" 
