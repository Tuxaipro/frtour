...
<div class="space-y-6">
    ...
    @if($reviews->total() > 0 || request()->has('search') || request()->has('status') || request()->has('rating'))
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                <div class="flex items-center space-x-3 mb-3">
                                    <div class="w-12 h-12 rounded-full bg-primary-100 flex items-center justify-center text-primary-600 font-semibold text-base flex-shrink-0">
                                        {{ $review->avatar_initials }}
                                    </div>
                                    <div>
                                        <h3 class="text-lg font-semibold text-slate-800">{{ $review->name }}</h3>
                                        @if($review->circuit)
                                            <p class="text-sm text-slate-500">{{ $review->circuit }}</p>
                                        @endif
                                    </div>
                                </div>
                                <div class="flex items-center gap-1 mb-3">
                                    ...
                                    @endfor
                                    <span class="text-sm text-slate-600 ml-2">{{ $review->rating }}/5</span>
                                </div>
                                <p class="text-slate-600 mb-3">{{ Str::limit($review->comment, 200) }}</p>
