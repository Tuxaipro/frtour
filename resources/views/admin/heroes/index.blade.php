...
@section('subtitle', 'Manage hero sections for the homepage')
...
<div class="space-y-6">
    ...
    @if($heroes->count() > 0)
        ...
        <form method="GET" action="{{ route('admin.heroes.index') }}" class="bg-white rounded-2xl shadow-lg border border-slate-200/50 p-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by title, subtitle, badge..." class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-all duration-200 text-slate-900 placeholder-slate-400">
                ...
                <div class="flex space-x-3">
                    ...
                    <a href="{{ route('admin.heroes.index') }}" class="flex-1 bg-white hover:bg-slate-50 text-slate-700 border-2 border-slate-200 px-5 py-3 rounded-xl font-semibold transition-all duration-200 text-center shadow-sm hover:shadow-md">Clear</a>
                </div>
            </div>
        </form>
        <div class="bg-white rounded-2xl shadow-lg border border-slate-200/50 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white">
                <h2 class="text-lg font-bold text-slate-900">All Hero Sections</h2>
                <p class="text-xs text-slate-600 mt-1">Total: {{ $heroes->count() }} hero section(s)</p>
            </div>
                ...
                <tbody class="bg-white divide-y divide-slate-200">
                    @foreach($heroes as $hero)
                        <tr class="hover:bg-primary/5 transition-colors duration-150">
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-slate-900">{{ $hero->title }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-slate-500">{{ Str::limit($hero->subtitle, 40) }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-slate-500">{{ $hero->badge_text ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                                {{ $hero->sort_order ?? 0 }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $hero->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <form action="{{ route('admin.heroes.duplicate', $hero) }}" 
                                          method="POST" 
                                          class="inline">
                                        @csrf
    ...
    @else
        <div class="bg-white rounded-2xl shadow-lg border border-slate-200/50 p-12 text-center">
            <div class="max-w-md mx-auto">
                ...
                <h3 class="text-xl font-bold text-slate-900 mb-2">No hero sections found</h3>
                <p class="text-sm text-slate-600 mb-8">Start by creating a hero section for your homepage.</p>
                <a href="{{ route('admin.heroes.create') }}" 
